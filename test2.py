...
print("sjekk1", len(ais_test['vesselId']))
...
print("sjekk2", len(ais_test['vesselId']))
ais_test = pd.merge(ais_test, schedules, left_on=['vesselId', 'day', 'month', 'year'], right_on=['vesselId', 'day', 'month', 'year'], how='left')
#ais_test = ais_test[['vesselId', 'hour', 'day', 'month', 'year', 'minute', 'yearBuilt', 'length', 'vesselType', 'breadth', 'CEU', 'DWT', 'GT', 'portLatitude', 'portLongitude']].drop_duplicates()
...
model = XGBRegressor(n_estimators=30000, random_state=rs)
