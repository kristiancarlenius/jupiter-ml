...
#ais_train[['sch_minute']].to_csv('x_data_7.csv', sep=',', index=False)
#ais_train[['yearBuilt']].to_csv('x_data_8.csv', sep=',', index=False)
#ais_train[['sch_hour']].to_csv('x_data_9.csv', sep=',', index=False)
...
model = ExtraTreesRegressor(n_estimators=135, random_state=rs)
...
predictions_df.to_csv('ais_test_predictions_3.csv', index=True)
print(datetime.datetime.today().strftime("%HH:%MM %dd"))